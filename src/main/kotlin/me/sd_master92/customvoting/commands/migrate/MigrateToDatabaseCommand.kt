...
class MigrateToDatabaseCommand(private val plugin: CV) : SimpleSubCommand("playerfiles")
{
                ...
                if (to == "database")
                {
                    if (plugin.hasDatabaseConnection())
                    {
                        object :
                            ConfirmGUI(
                                plugin,
                                PMessage.MIGRATE_VOTES_COMMAND_INVENTORY_NAME_TO_DATABASE_CONFIRM.toString()
                            )
                        {
                            ...
                            override fun onConfirm(event: InventoryClickEvent, player: Player)
                            {
                                ...
                                plugin.launch {
                                    try
                                    {
                                        val players = VoteFile.getAll()
                                        total = players.size
                                        for (existing in players)
                                        {
                                            val voter = PlayerTable(plugin, existing.getUuid())
                                        ...
                                        player.sendMessage(PMessage.MIGRATE_VOTES_MESSAGE_TO_DATABASE_SUCCESS_X.with("" + players.size))
                                    } catch (_: Exception)
                                    {
                                        player.sendMessage(PMessage.MIGRATE_VOTES_MESSAGE_FAILURE.toString())
                                    } finally
