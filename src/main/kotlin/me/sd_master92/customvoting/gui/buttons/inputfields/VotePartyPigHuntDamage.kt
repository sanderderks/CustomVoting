...
class VotePartyPigHuntDamage(
    private val plugin: CV,
    private val currentPage: GUI
) : BaseItem(VMaterial.PIG_HEAD.get(), PMessage.VOTE_PARTY_ITEM_NAME_PIG_HUNT_DAMAGE.toString())
{
    val path = Setting.VOTE_PARTY_PIG_HUNT_DAMAGE_MAX.path
    ...
    override fun onClick(event: InventoryClickEvent, player: Player)
    {
        ...
        object : PlayerNumberInput(plugin, player)
        {
            override fun onNumberReceived(input: Int)
            {
                ...
                event.currentItem = VotePartyPigHuntDamage(plugin, currentPage)
